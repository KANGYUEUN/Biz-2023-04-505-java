 ...
 * 각 method 에 Annotaion을 부착하여 SQL 명령문을 설정한다
 * 
 * 그러면 나머지 여러가지 코드 DBMS 에서 데이터를 가져와서 DTO 에 담고, 
 * 다시 List 에 add 하여 return 하는 코드를 Mybatis 의 sqlSessionFatory 가 대신 생성해 준다.
 * 
 */
public interface StudentDao {
	
	@Select(" SELECT * FROM tbl_student ")
	public List<StudentDto> seletAll();
	
	@Select(" SELECT * FROM tbl_student WHERE st_num = #{id} ")
	public StudentDto findById(String id);
	
	// name 은 복수이므로 List 타입으로 받아야 함.
	@Select(" SELECT * FROM tbl_student WHERE st_name = #{id} ")
